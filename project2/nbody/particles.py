...
class Particles:
    ...
    def output(self, filename):
        ...
        data = [['#time[s]', 'tag', 'mass[kg]', 'x[m]', 'y[m]', 'z[m]',
                 'vx[m/s]', 'vy[m/s]', 'vz[m/s]', 'ax[m/s^2]', 'ay[m/s^2]', 'az[m/s^2]']]
        for i in range(len(tags)):
            data.append([time, tags[i], masses[i], *pos[i*3:i*3+3], *vel[i*3:i*3+3], *acc[i*3:i*3+3]])

        # write down the list to a file
        with open(filename, 'w') as f:
            for row in data:
                formatted_row = [value.rjust(15) for value in row]
